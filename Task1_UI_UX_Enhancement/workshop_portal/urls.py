...
urlpatterns = [
    ...
    # Redirect default auth login to custom workshop login (place before include)
    url(r'^reset/login/$', RedirectView.as_view(url='/workshop/login/', permanent=False)),
    url(r'^reset/login$', RedirectView.as_view(url='/workshop/login/', permanent=False)),
    url(r'^reset/$', RedirectView.as_view(url='/workshop/login/', permanent=False)),
